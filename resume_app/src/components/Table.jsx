...
export default function Table({ columns, rows, headerStyle, pagination }) {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = pagination || 50;
            ...
            <tr>
              {columns.map((column, index) => (
                <th
                  key={column.id}
                  scope="col"
                  className={`px-6 py-4 font-[600]   tracking-wider ${headerStyle[index]} `}
                >
                  {column.name}
                  {/*  different style for all cells */}
                </th>
              ))}
            </tr>
          </thead>
